use serde::Serialize;

#[derive(Debug, Clone, Serialize)]
pub struct ThumbnailInfo {
    pub index: usize,
    pub frame_index: usize,
    pub pts: Option<f64>,
    pub width: u32,
    pub height: u32,
    pub size_bytes: usize,
}

pub struct ThumbnailExtractor {
    pub thumbnails: Vec<ThumbnailEntry>,
    interval_frames: usize,
    frame_count: usize,
}

pub struct ThumbnailEntry {
    pub info: ThumbnailInfo,
    pub jpeg_data: Vec<u8>,
}

impl ThumbnailExtractor {
    pub fn new(interval_frames: usize) -> Self {
        Self {
            thumbnails: Vec::new(),
            interval_frames: interval_frames.max(1),
            frame_count: 0,
        }
    }

    pub fn should_capture(&self) -> bool {
        self.frame_count % self.interval_frames == 0
    }

    pub fn add_placeholder(&mut self, frame_index: usize, pts: Option<f64>, width: u32, height: u32) {
        let idx = self.thumbnails.len();
        // generate a simple placeholder JPEG (1x1 gray pixel)
        let jpeg = minimal_jpeg(width.min(320), height.min(180));
        let size = jpeg.len();
        self.thumbnails.push(ThumbnailEntry {
            info: ThumbnailInfo {
                index: idx,
                frame_index,
                pts,
                width: width.min(320),
                height: height.min(180),
                size_bytes: size,
            },
            jpeg_data: jpeg,
        });
    }

    pub fn inc_frame(&mut self) {
        self.frame_count += 1;
    }

    pub fn list(&self) -> Vec<ThumbnailInfo> {
        self.thumbnails.iter().map(|t| t.info.clone()).collect()
    }
}

fn minimal_jpeg(w: u32, h: u32) -> Vec<u8> {
    // minimal valid JPEG: SOI + APP0 + DQT + SOF0 + DHT + SOS + EOI
    // for simplicity, return a 1x1 gray JPEG regardless of requested size
    let _ = (w, h);
    vec![
        0xFF, 0xD8, // SOI
        0xFF, 0xE0, 0x00, 0x10, 0x4A, 0x46, 0x49, 0x46, 0x00, 0x01, 0x01, 0x00, 0x00, 0x01, 0x00, 0x01, 0x00, 0x00, // APP0
        0xFF, 0xDB, 0x00, 0x43, 0x00, // DQT
        0x08, 0x06, 0x06, 0x07, 0x06, 0x05, 0x08, 0x07, 0x07, 0x07, 0x09, 0x09, 0x08, 0x0A, 0x0C, 0x14, 0x0D, 0x0C, 0x0B, 0x0B, 0x0C, 0x19, 0x12, 0x13, 0x0F, 0x14, 0x1D, 0x1A, 0x1F, 0x1E, 0x1D, 0x1A, 0x1C, 0x1C, 0x20, 0x24, 0x2E, 0x27, 0x20, 0x22, 0x2C, 0x23, 0x1C, 0x1C, 0x28, 0x37, 0x29, 0x2C, 0x30, 0x31, 0x34, 0x34, 0x34, 0x1F, 0x27, 0x39, 0x3D, 0x38, 0x32, 0x3C, 0x2E, 0x33, 0x34, 0x32,
        0xFF, 0xC0, 0x00, 0x0B, 0x08, 0x00, 0x01, 0x00, 0x01, 0x01, 0x01, 0x11, 0x00, // SOF0
        0xFF, 0xC4, 0x00, 0x1F, 0x00, 0x00, 0x01, 0x05, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, // DHT DC
        , // DHT AC
        0xFF, 0xDA, 0x00, 0x08, 0x01, 0x01, 0x00, 0x00, 0x3F, 0x00, 0x7B, 0x40, // SOS
        0xFF, 0xD9, // EOI
    ]
}

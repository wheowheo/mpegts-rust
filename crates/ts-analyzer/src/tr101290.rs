...
impl Tr101290Checker {
    ...
    pub fn check_pat(&mut self, pid: u16, is_pat: bool, packet_index: u64) {
        ...
        if pid == 0x0000 || packet_index % 10000 == 0 {
            if self.last_pat_index.is_none() && packet_index > 5000 && self.packet_rate > 0.0 {
    ...
    pub fn check_nit(&mut self, is_nit: bool, packet_index: u64) {
        if is_nit { self.last_nit_index = Some(packet_index); }
    }

    pub fn check_sdt(&mut self, is_sdt: bool, packet_index: u64) {
        if is_sdt { self.last_sdt_index = Some(packet_index); }
    }
    ...
    fn push_error(&mut self, err: Tr101290Error) {
    ...
    pub fn summary(&self) -> Tr101290Summary {
        Tr101290Summary {
            p1_count: self.p1_counters.values().sum(),
            p2_count: self.p2_counters.values().sum(),
            p3_count: self.p3_counters.values().sum(),
    ...
    pub fn summary_counts_only(&self) -> (u64, u64, u64) {
        (
            self.p1_counters.values().sum(),
            self.p2_counters.values().sum(),
            self.p3_counters.values().sum(),
        )
    }
    ...
    pub fn counters(&self) -> (&HashMap<String, u64>, &HashMap<String, u64>, &HashMap<String, u64>) {
        (&self.p1_counters, &self.p2_counters, &self.p3_counters)
    }

    pub fn errors_slice(&self) -> &VecDeque<Tr101290Error> {
        &self.errors
    }
}
